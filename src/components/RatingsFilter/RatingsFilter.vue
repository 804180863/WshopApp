  ...
  <div class="ratingselect">
    <div class="rating-type border-1px">
      ...
      <span class="block" :class="{active: selectType===0}" @click="setSelectType(0)">
        推荐<span class="count">{{totalUpRatingCount}}</span>
      </span>
      <span class="block" :class="{active: selectType===1}" @click="setSelectType(1)">
        吐槽<span class="count">{{totalRatingCount-totalUpRatingCount}}</span>
      </span>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['totalRatingCount', 'totalUpRatingCount'])
    },
    methods: {
      setSelectType (type) {
        // 分发vue的自定义事件: setSelectType
        this.$emit('setSelectType', type)
      },
      toggleConlyContent () {
        // 分发vue的自定义事件: toggleOnlyCotent
        this.$emit('toggleOnlyCotent')
      }
    }
  }
</script>
